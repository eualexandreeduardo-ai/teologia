(() =>{
 ...
 const approved = moduleApproved(n);
 status.innerHTML = `<div class="module-status-strip ${approved ? 'approved' : ''}"> <div><strong>Status do Módulo ${String(n).padStart(2,'0')}</strong><span>Checklist: ${taskPct}% · Avaliação: ${qPassed ? 'aprovada' : 'pendente'}</span></div> <div class="status-pill">${approved ? ' Aprovado' : ' Em andamento'}</div> </div>`;
 }
 ...
 <label class="quiz-option"> <input type="radio" name="quiz-m${n}-q${idx}" value="${oi}" ${saved === String(oi) ? 'checked' : ''}> <span>${op}</span> </label>`).join('');
 return `<div class="quiz-question" data-question="${idx}"> <h5>${idx+1}. ${item.q}</h5> <div class="quiz-options">${opts}</div> </div>`;
 }).join('');
